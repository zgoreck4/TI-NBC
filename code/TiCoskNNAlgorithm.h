#ifndef TiCoskNNAlgorithm_H
#define TiCoskNNAlgorithm_H
...
class TiCoskNNAlgorithm
{
public:
    TiCoskNNAlgorithm(int k, std::vector<DataSample> data, std::chrono::microseconds::rep& sort_time, std::vector<double> ref_coords);
    ~TiCoskNNAlgorithm();
...
private:
    ...
    double findMin(std::vector<DistanceFrom> vec);
    int calcNoMaxNeigh();
    void convert_eps_to_cos();
    void convert_eps_from_cos();
